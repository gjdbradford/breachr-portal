    ...
    <div style={{ padding: '0 24px 48px' }}>
      ...
      <div className="gs" style={{
        ...
        <p style={{ fontSize: 14, color: '#94a3b8', maxWidth: 520, margin: '0 auto 28px', lineHeight: 1.7, position: 'relative' }}>
          A Breachr sensor is a lightweight Docker container that runs silently inside your network. It discovers every connected device, maps open ports, and checks them against known vulnerabilities — giving you a live, risk-scored asset inventory without touching a single device.
        </p>
        <button onClick={() => onAddSensor(selectedType ?? 'docker')} className="btn-p" style={{ fontSize: 14, padding: '10px 28px', position: 'relative' }}>
      ...
      <div className="gs" style={{ padding: '28px 32px', marginBottom: 24 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: '#e2e8f0', marginBottom: 24, letterSpacing: '0.04em' }}>
          HOW TO DEPLOY A SENSOR — 3 STEPS
        </h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {[
            {
              n: '1',
              title: 'Register the sensor in this portal',
              body: 'Click "Add sensor" above. Enter a name (e.g. "Office London") and an optional location. Click Register. You\'ll receive a one-time token — copy it before closing.',
              note: null,
            },
            {
              n: '2',
              title: 'Run the Docker container inside your network',
              body: 'On any Linux machine on the same subnet as the devices you want to discover, run:',
              note: 'docker run -d --network host \\\n  -e BREACHR_SENSOR_TOKEN=<your-token> \\\n  -e BREACHR_SENSOR_ID=<your-sensor-id> \\\n  -e BREACHR_API_URL=https://breachr-portal.vercel.app \\\n  ghcr.io/gjdbradford/sensor:latest',
            },
            {
              n: '3',
              title: 'Watch devices appear',
              body: 'Within 60–90 seconds, devices that have communicated on the network will start appearing in the Inventory page. The sensor phones home every 60 seconds with its latest findings.',
              note: null,
            },
          ].map((step, i, arr) => (
            <div key={step.n} style={{ display: 'flex', gap: 20, paddingBottom: i < arr.length - 1 ? 28 : 0 }}>
        ...
        <div style={{ marginTop: 28, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <p style={{ fontSize: 11, color: '#475569', lineHeight: 1.6 }}>
            <strong style={{ color: '#64748b' }}>Prerequisites:</strong> A Linux host (Raspberry Pi, VM, or server) on the same subnet as your devices. Docker installed. Outbound HTTPS to the portal URL.
            The <code style={{ background: 'rgba(255,255,255,0.06)', padding: '1px 5px', borderRadius: 3 }}>--network host</code> flag is required for ARP sniffing on Linux.
          </p>
        </div>
      </div>
