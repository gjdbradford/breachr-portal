...
const STATUSES = ['open', 'in_progress', 'remediated'] as const
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    width: '100%', padding: '9px 12px', fontSize: 12, cursor: 'pointer',
                    background: s === status ? 'rgba(255,255,255,0.04)' : 'transparent',
                    border: 'none', color: ss.color, textAlign: 'left',
                  }}
                >
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: ss.color, flexShrink: 0 }} />
                  {STATUS_LABEL[s]}
                  {s === status && <span style={{ marginLeft: 'auto', fontSize: 10 }}>✓</span>}
                </button>
              )
            })}
          </div>
